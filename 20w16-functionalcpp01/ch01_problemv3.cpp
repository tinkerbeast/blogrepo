...
auto word_frequencies_odd(const std::vector<std::string>& files) {
    ...
    for (const auto& file_name: files) {
        ...
        std::string word;
        while (in >> word) {
            if (word.size() % 2 != 0) { // only one condition change
                ++word_freq[word];
            }           
        }
    }
...
auto string_is_odd(const std::string& str) {
    return str.size() % 2 != 0;
}
...
auto word_frequencies_functional(const std::vector<std::string>& files,
    ...
    std::copy_if(words.begin(), words.end(), 
            std::back_inserter(filtered_words), filter_func);
...
auto word_frequencies_even_functional(const std::vector<std::string>& files) {
    return word_frequencies_functional(files, string_is_even);
}

auto word_frequencies_odd_functional(const std::vector<std::string>& files) {
    return word_frequencies_functional(files, string_is_odd);
}
...
int main() {
    ...
    auto&& imperative_result = word_frequencies_odd(files);
    std::cout << "Top 5 odd results by imperative code:\n";
    print_top(imperative_result, 5);
    
    auto&& functional_result = word_frequencies_odd_functional(files);
    std::cout << "Top 5 odd results by functional code:\n";
