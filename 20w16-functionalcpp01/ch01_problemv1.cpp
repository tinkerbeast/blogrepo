...
auto word_frequencies(const std::vector<std::string>& files) {
...
auto word_frequencies_functional(const std::vector<std::string>& files) {
    ...
    auto&& words = std::accumulate(vec_words.begin(), vec_words.end(), nowords, flatten);
...
int main() {
    ...
    auto&& imperative_result = word_frequencies(files);
    std::cout << "Top 5 results by imperative code:\n";
    print_top(imperative_result, 5);
    
    auto&& functional_result = word_frequencies_functional(files);
    std::cout << "Top 5 results by functional code:\n";
